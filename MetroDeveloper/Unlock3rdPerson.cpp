...
Unlock3rdPerson::Unlock3rdPerson()
{
	if (Utils::GetBool("other", "unlock_3rd_person_camera", false)) {
		...
		if (Utils::isRedux()) {
			...
			base_npc_cameras_cam_set = (_base_npc_cameras_cam_set)FindPatternInEXE(
				"",
			...
			if (Utils::isExodusPatched) {
				...
				base_npc_cameras_cam_set = (_base_npc_cameras_cam_set)FindPatternInEXE(
					"\x48\x89\x5C\x24\x00\x48\x89\x74\x24\x00\x48\x89\x7C\x24\x00\x41\x56\x48\x83\xEC\x30\x48\x63\x81",
		...
		if (Utils::isLL()) {
			...
			base_npc_cameras_cam_set = (_base_npc_cameras_cam_set)FindPatternInEXE(
				"\x53\x56\x8B\xF1\x8B\x46\x4C\x57\x8B\x7C\x86\x34\x8B\x44\x24\x10\x89\x46\x4C\x8B\x17\x8B\x5C\x86\x34\x8B\x42\x14\x8B\xCF",
				"xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
		} else {
			// B8 ? ? ? ? 84 05 ? ? ? ? 56 75 1D 09 05 ? ? ? ? 68 ? ? ? ? C7 05 ? ? ? ? ? ? ? ? E8 ? ? ? ? 83 C4 04 8B 47 6C - ORIG 2033
			set_camera_2033 = (_set_camera_2033)FindPatternInEXE(
				"\xB8\x00\x00\x00\x00\x84\x05\x00\x00\x00\x00\x56\x75\x1D\x09\x05\x00\x00\x00\x00\x68\x00\x00\x00\x00\xC7\x05\x00\x00\x00\x00\x00\x00\x00\x00\xE8\x00\x00\x00\x00\x83\xC4\x04\x8B\x47\x6C",
...
void Unlock3rdPerson::clevel_r_on_key_press(void* _this, int action, int key, int state, int resending)
{
	if (key <= 61 && key >= 59) {
		...
		if (Utils::isRedux()) {
			...
			if (key == 59) // F1
				base_npc_cameras_cam_set(base_npc_cameras, enc_first_eye_old, 1.f, 1);
			if (key == 60) // F2
				base_npc_cameras_cam_set(base_npc_cameras, enc_look_at_old, 1.f, 1);
			if (key == 61) // F3
				base_npc_cameras_cam_set(base_npc_cameras, enc_free_look_old, 1.f, 1);
			...
			base_npc_cameras = (Utils::isArktika() ? *((void**)((char*)control_entity + 0x7C8)) : *((void**)((char*)control_entity + base_npc_cameras_exodus_offset)));

			if (key == 59) // F1
				base_npc_cameras_cam_set(base_npc_cameras, enc_first_eye_new, 1.f, 1);
			if (key == 60) // F2
				base_npc_cameras_cam_set(base_npc_cameras, enc_look_at_new, 1.f, 1);
			if (key == 61) // F3
				base_npc_cameras_cam_set(base_npc_cameras, enc_free_look_new, 1.f, 1);
		}
#else
		if (Utils::isLL()) {
			...
			if (key == 59) // F1
			{
				__asm
				{
					...
					push enc_first_eye_old      // camera_style = enc_first_eye
			...
			if (key == 60) // F2
			{
				...
				{
					push 3F800000h              // speed = 1.f
					push enc_look_at_old        // camera_style = enc_look_at
					mov edi, [base_npc_cameras] // 'this' pointer passed in EDI
					call[set_camera_2033]
				}
			}
			...
			if (key == 61) // F3
			{
				...
				{
					push 3F800000h              // speed = 1.f
					push enc_free_look_old      // camera_style = enc_free_look
					mov edi, [base_npc_cameras] // 'this' pointer passed in EDI
					call[set_camera_2033]
				}
			}
