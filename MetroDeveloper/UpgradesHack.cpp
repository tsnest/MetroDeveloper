...
UpgradesHack::UpgradesHack()
{
	...
	// нужно их отключить, иначе ресурсы не подойдут при запуске с ключом -build_key m3
	...
	if (Utils::GetBool("arktika1", "upgrades_hack", false)) {
		...
		DWORD64 addr1 = FindPatternInEXE(
			"\x0f\x85\x00\x00\x00\x00\x48\x8b\x15\x00\x00\x00\x00\x48\x85\xd2\x74\x00\x49\x8b\xc6\x4c\x8d\x05\x00\x00\x00\x00\x0f\xb6\x4c\x02",
			"xx????xxx????xxxx?xxxxxx????xxxx");

		BYTE mem1[] = { 0xE9, 0xC6, 0x02, 0x00, 0x00, 0x90 };
		ASMWrite((void*)addr1, mem1, sizeof(mem1));
		...
		DWORD64 addr2 = FindPatternInEXE("\x48\x0f\x44\xc2\x48\x8d\x4c\x24\x00\xba", "xxxxxxxx?x");

		BYTE mem2[] = { 0x90, 0x90, 0x90, 0x90 };
		ASMWrite((void*)addr2, mem2, sizeof(mem2));
		...
		DWORD64 addr3 = FindPatternInEXE("\x0f\x94\xc0\x85\xc0\x74\x00\x48\x8b\x03\x48\x85\xc0", "xxxxxx?xxxxxx");

		BYTE mem3[] = { 0x0F, 0x95, 0xC0 };
		ASMWrite((void*)addr3, mem3, sizeof(mem3));
	}
}
